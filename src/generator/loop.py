...
def _propose(client, prompt: str, retries: int, seed=None):
    # The retry seed shifts too: with one fixed seed a malformed response would be
    # regenerated byte for byte on every attempt, so the retry could never succeed.
...
def propose_revision(client, params, est, target, template, duration, retries=3, seed=None):
    ...
    def run(self, brief, run_idx, out_dir):
        ...
        for it in range(self.cap):
            ...
            revised = propose_revision(self.client, params, est, target,
                                       ...
                                       seed=call_seed(brief["id"], run_idx, it))
            if revised is None:
                break
            params = revised
